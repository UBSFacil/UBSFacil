...
// IMPORTANTE: o backend (main.py) NÃO usa prefixo /api em nenhuma rota
// (ex: POST /auth/login, GET /auth/me). Não adicione /api aqui.
...
api.interceptors.response.use(
  ...
  (error) => {
    if (error.response?.status === 401) {
      // Token expirado ou inválido: limpa a sessão local.
      // Não forçamos redirect aqui — quem decide se a página exige
      // login é o ProtectedRoute (rotas como /admin), não a API.
      // Um redirect global (window.location.href) chutava a pessoa
      // pra /login mesmo em páginas públicas, como a Home.
