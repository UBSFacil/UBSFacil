...
from .models import Medicamento, Unidade, Estoque, Usuario
...
def seed_admin(db: Session):
    """Garante que exista um usuário ADM de teste para o botão 'Entrar como ADM'."""
    if db.query(Usuario).filter(Usuario.email == "adm@ubsfacil.com").first():
        return

    admin = Usuario(
        nome="Administrador UBSFacil",
        email="adm@ubsfacil.com",
        sus_cartao="000000000000000",
...
def listar_unidades(db: Session = Depends(get_db)):
    """Alias de /ubs usado pelo painel ADM (unidadesService.buscarTodas)."""
    return [
        {"id": u.id, "nome": u.nome, "endereco": u.endereco, "bairro": getattr(u, "bairro", None)}
